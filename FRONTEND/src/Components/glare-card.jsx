...
export const GlareCard = ({ children, className }) => {
  ...
  const containerStyle = {
    ...
    "--woodenColor": "rgba(245, 222, 179, 0.8)",
  ...
  const backgroundStyle = {
    ...
    "--woodenEffect": `linear-gradient(45deg, var(--woodenColor) 50%, transparent 50%)`,
  ...
  return (
        ...
        if (refElement.current) {
          setTimeout(() => {
            if (isPointerInside.current) {
              ...
              refElement.current.style.setProperty("--wooden-angle", "45deg");
    ...
    >
      <div className="h-full grid will-change-transform origin-center transition-transform duration-[var(--duration)] ease-[var(--easing)] delay-[var(--delay)] [transform:rotateY(var(--r-x))_rotateX(var(--r-y))] rounded-[var(--radius)] border border-neutral-600 hover:[--opacity:0.6] hover:[--duration:200ms] hover:[--easing:linear] hover:filter-none overflow-hidden bg-transparent">
        <div className="w-full h-full grid [grid-area:1/1] mix-blend-soft-light bg-transparent [clip-path:inset(0_0_0_0_round_var(--radius))]">
          <div
            className={cn(
              "h-full w-full bg-gradient-to-tl from-[#663300] to-transparent ",
              className
            )}
          >
            {children}
